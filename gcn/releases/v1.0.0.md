...
- N64 controller support via passive adapter (libogc2 SI detection):
  ...
  Memory Pak, Rumble Pak, Transfer Pak, Bio Sensor (with BPM readout),
  Snap Station. Rumble actuation on hold-A for both GCN's built-in
  motor and N64's Rumble Pak.
...
- Idle screensaver after 30s: bouncing joypad logo bitmap with a
  7-color cycle (red / green / yellow / blue / magenta / cyan / white)
  on each wall bounce. Any input wakes back to the live view.
